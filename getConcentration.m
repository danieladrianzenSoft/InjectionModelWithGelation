function [DYDT] = getConcentration(t, Y, r, v_spline, dvdr_spline, rc_spline, p_spline, ec_spline, decdr_spline, params)
    ...
    if t <= t_inj_end
        ...
        for i = ind_r0 + 1
            ...
            dYdt(i,:) = ((2*D/r(i)).*dYdr(i,:)) + (D*d2Ydr2(i,:)) - ((2/r(i)).*(v.*Y(i,:))) - (Y(i,:).*dvdr) - (v.*dYdr(i,:));
        end
...
function D_C = get_DC(u,D_Cmin,D_Cmax)
    if u > 1
        u = 1;
    elseif u < -1
        u = -1;

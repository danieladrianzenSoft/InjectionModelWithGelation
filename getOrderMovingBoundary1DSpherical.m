function out = getOrderMovingBoundary1DSpherical(t,u,r,rc_spline,D,gamma,C0,options) 
    arguments
       t
       u
       r
...
%         ruMuLaplace(domain_inds,:) = 2*ruMuDuDr(domain_inds,:)./r(domain_inds)' + (ruMu(domain_inds-1,:) - 2*ruMu(domain_inds,:) + ru_m(domain_inds+1,:)) ./ dr.^2;
    ...
    ruLaplace(domain_inds,:) = 2*(ruDuDr(domain_inds,:)./r(domain_inds)') + (dr_p.*ru_m(domain_inds-1,:)-(dr_p+dr_n).*ru_m(domain_inds,:)+dr_n.*ru_m(domain_inds+1,:)) ./ ((1/2)*(dr_p+dr_n).*dr_p.*dr_n);
...
% 
        ...
        ruLaplace(interior_bound,:) = 2*ruDuDr(interior_bound,:)./r(interior_bound)' + (2*(dr_p./(dr_p+dr_n)).*(ru_m(interior_bound-1,:)) - 2*ru_m(interior_bound,:) + (2*(dr_n./(dr_p+dr_n))*C0)) ./ (dr_p.*dr_n);  % nabla2C = 2/r dC/dr + d2C/dr2
        ...
        ruMuLaplace(interior_bound,:) = 2*ruMuDuDr(interior_bound,:)./r(interior_bound)' + (dr_p.*ruMu(interior_bound-1,:) - (dr_p+dr_n).*ruMu(interior_bound,:) + dr_n.*ru_m(interior_bound+1,:)) ./ ((1/2)*(dr_p+dr_n).*dr_p.*dr_n);
